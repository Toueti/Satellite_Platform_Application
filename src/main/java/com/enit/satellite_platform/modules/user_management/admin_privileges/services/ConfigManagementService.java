...
public class ConfigManagementService {
    ...
    public ConfigManagementService() {
        ...
        properties.put("image.storage.path", "Path for storing images");
        ...
        properties.put("messaging.attachments.allowed.types", "Allowed attachment file formats (comma-separated list of extensions or MIME types)");
        // properties.put("messaging.attachments.storage.type", "Where to store attachments (local or cloud) - Future use"); // Example for future
        // properties.put("messaging.attachments.storage.path", "Base file storage directory (if using local storage) - Future use, currently uses file.upload-dir"); // Example for future

        // Messaging Configuration - Retention & Cleanup
        properties.put("messaging.retention.days", "How long messages are stored before deletion (0 to disable retention-based cleanup)");
        properties.put("messaging.cleanup.enabled", "Enable/disable automatic message cleanup job");
        properties.put("messaging.max.messages.per.conversation", "Maximum messages per conversation before oldest are deleted (0 to disable count-based cleanup)");
    ...
    public ConfigProperty updateProperty(UpdatePropertyRequestDto request) {
        ...
        // Fetch the original default value from the environment *before* applying the potential new override.
        ...
        ConfigProperty property = configPropertyRepository.findById(key)
                .orElse(new ConfigProperty(key, null, MANAGEABLE_PROPERTIES.get(key), originalDefaultValue)); // Pass default value

        // Ensure the defaultValue field is populated if the entity already exists but lacks it
        ...
        if (newValue == null) { // Resetting to default
            if (configPropertyRepository.existsById(key)) {
                ...
                // Return a representation of the reset state, using the fetched original default
                ...
                // Ensure the existing property representation has the default value set correctly
    ...
    private void validatePropertyValue(String key, String value) {
        ...
        try {
            switch (key) {
                ...
                case "logging.level.org.springframework":
                    if (!VALID_LOG_LEVELS.contains(value.toUpperCase())) {
                        throw new IllegalArgumentException("Invalid log level '" + value + "' for '" + key + "'. Must be: " + VALID_LOG_LEVELS);
                    }
                    break;
                case "spring.data.mongodb.auto-index-creation":
                    if (!"true".equalsIgnoreCase(value) && !"false".equalsIgnoreCase(value)) {
                        throw new IllegalArgumentException("Invalid boolean value '" + value + "' for '" + key + "'. Must be 'true' or 'false'.");
                    }
                    break;
                case "jwt.expiration":
                ...
                case "springdoc.swagger-ui.path":
                    if (!value.matches("/[a-zA-Z0-9/-]*")) {
                        throw new IllegalArgumentException("Swagger UI path '" + value + "' must start with '/' and contain only letters, numbers, slashes, or hyphens.");
                    }
                    break;
                ...
                case "cache.cleanup.cron":
                    if (!CronExpression.isValidExpression(value)) {
                        throw new IllegalArgumentException("Invalid cron expression '" + value + "' for '" + key + "'.");
                    }
                    break;
                case "jwt.secret":
                    if (value.length() < 16 || value.matches("[a-zA-Z0-9]+")) {
                        throw new IllegalArgumentException("JWT secret must be at least 16 characters and include special characters.");
                    }
                    break;
                case "cache.redis.prefix":
                    if (!value.matches("[a-zA-Z0-9:_-]+")) {
                        throw new IllegalArgumentException("Redis prefix must contain only letters, numbers, colons, underscores, or hyphens.");
                    }
                    break;
                ...
                case "messaging.attachments.allowed.types":
                    if (value.isBlank() || Arrays.stream(value.split(",")).allMatch(String::isBlank)) {
                        throw new IllegalArgumentException("Property '" + key + "' must contain a non-empty comma-separated list of types/extensions.");
                    }
                ...
                case "messaging.user-to-bot.enabled":
                    ...
                    break;
                // case "messaging.max.messages.per.user.daily": // Add later if implementing rate limiting
                ...
                case "messaging.rabbitmq.routing.bot.topic":
                     // Basic validation: not blank. More specific routing key validation could be added.
                     if (value.isBlank()) {
            ...
            throw new IllegalArgumentException("Invalid URI format '" + value + "' for '" + key + "': " + e.getMessage());
