...
export default function AnalysisPage() {
  ...
  const [parameters, setParameters] = useState<GeeParams>({
    startDate: '',
    endDate: '',
    region: '',
    analysisType: '',
  ...
  const [error, setError] = useState('');
    const [result, setResult] = useState<string | null>(null); // To store a result message or URL
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Basic validation
    if (!selectedProject || !analysisType || !parameters.startDate || !parameters.endDate || !parameters.region) {
    ...
    try {
      const geeResult = await geeService.process(parameters);
            // Assuming the result contains a URL or message
            setResult(geeResult.imageUrl || 'Analysis completed successfully!');
            ...
            <div>
              ...
              <textarea
                ...
                onChange={(e) => setParameters({ ...parameters, region: e.target.value })}
                                ...
                                className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-primary-600 focus:border-primary-600"
                            /> {/* TODO: Integrate a map for region selection */}
                        </div>
